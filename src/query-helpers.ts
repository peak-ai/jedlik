...
function getAttributeNamesFromFilter<T>({
  key,
}: Filter<T>): ExpressionAttributeNameMap {
  return {
    [`#${key}`]: key,
  };
}

function getAttributeNamesFromFilterMaps<T>(
  filters: FilterMap<T>[]
): ExpressionAttributeNameMap {
...
function getAttributeValuesFromFilter<T>({
  key,
  value,
}: Filter<T>): ExpressionAttributeValueMap {
  return {
    [`:${encode(`${key}${value}`)}`]: value,
  };
}

function getAttributeValuesFromFilterMaps<T>(
  filters: FilterMap<T>[]
): ExpressionAttributeValueMap {
  return filters.reduce(
    (names, filter) => ({
...
function getSimpleFilterExpression<T>(filter: Filter<T>): string {
  return `#${filter.key} ${filter.operator} :${encode(
...
export function getAttributeNamesFromFilters<T>(
  filters: FilterMap<T>
): ExpressionAttributeNameMap {
...
export function getAttributeValuesFromFilters<T>(
  ...
  if ((filters as FilterAndGroup<T>).$and) {
    return getAttributeValuesFromFilterMaps(
      (filters as FilterAndGroup<T>).$and
    );
  }
